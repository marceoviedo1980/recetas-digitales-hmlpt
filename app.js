...
function renderPreview() {
  ...
  // Ambulatorio e Internado en modo dos copias usan plantillas nativas de media hoja
  if (data.printLayout === "halfCopies" && state.template !== "utiUcin") {
    const copyHtml = state.template === "internado"
      ? renderHalfInternadoNative(data)
      : renderHalfAmbulatorioNative(data);
    preview.innerHTML = renderHalfSheetNative(copyHtml);
    return;
  }
...
// ============================================================
//  PLANTILLAS NATIVAS DE MEDIA HOJA (sin transform/zoom)
//  Ambulatorio e Internado — modo Dos copias
// ============================================================

function renderHalfSheetNative(copyHtml) {
  return `
    <div class="half-sheet-copy half-native">${copyHtml}</div>
    <div class="sheet-cut-line" aria-hidden="true"></div>
    <div class="half-sheet-copy half-native">${copyHtml}</div>
  `;
}

function hrCheck(label, checked) {
  return `<span class="hr-check">${safe(label)} <span class="hr-box">${checked ? "X" : ""}</span></span>`;
}

function hrDiagnosisBlock(data, maxSecondary) {
  const rows = Array.from({ length: maxSecondary }, (_, i) => {
    const n = i + 1;
    return { diag: data[`secondaryDiagnosis${n}`] || "", code: data[`secondaryCode${n}`] || "" };
  });
  return `
    <table class="hr-diag-table">
      <thead><tr><th>Diagnostico</th><th class="hr-cie-col">CIE-10</th></tr></thead>
      <tbody>
        <tr><td><strong>Principal:</strong> ${safe(data.diagnosis)}</td><td>${safe(data.cie10)}</td></tr>
        ${rows.map((r, i) => `<tr><td>${i + 1}. ${safe(r.diag)}</td><td>${safe(r.code)}</td></tr>`).join("")}
      </tbody>
    </table>
  `;
}

function renderHalfAmbulatorioNative(data) {
  ...
  return `
    <div class="hr-recipe hr-amb">
      <div class="hr-header">
        <div class="hr-brand">
          <span>Hospital Municipal La Portada</span>
          <span>Zona La Portada/ Av. La Florida</span>
          <span>Calle s/n Macrodistrito II</span>
        </div>
        <div class="hr-title">RECETARIO / RECIBO<br>ATENCION AMBULATORIA</div>
        <div class="hr-panel">
          ...
          <div><strong>PROGRAMA:</strong> <span>${safe(patientProgram)}</span></div>
          <div class="hr-panel-dates">
            <div class="hr-date-inline"><strong>F.NAC.</strong> <span>${safe(birthDate.day)}</span><em>/</em><span>${safe(birthDate.month)}</span><em>/</em><span>${safe(birthDate.year)}</span></div>
            <div class="hr-sex-inline"><strong>SEXO:</strong> <span class="hr-box">${data.sex === "M" ? "X" : ""}</span>M <span class="hr-box">${data.sex === "F" ? "X" : ""}</span>F</div>
          </div>
          <div class="hr-date-inline"><strong>F.SOLICITUD.</strong> <span>${safe(requestDate.day)}</span><em>/</em><span>${safe(requestDate.month)}</span><em>/</em><span>${safe(requestDate.year)}</span></div>
        </div>
      </div>
      <div class="hr-meta">
        <span><strong>SEDES:</strong> LA PAZ</span>
        <span><strong>RED:</strong> 2 NOR OESTE</span>
        <span><strong>Municipio:</strong> LA PAZ</span>
        <span><strong>Estab.:</strong> HOSPITAL MUNICIPAL LA PORTADA</span>
      </div>
      <div class="hr-attention-row">
        <strong>Tipo de Atencion:</strong>
        ${hrCheck("EN CONSULTORIO", data.attentionType === "EN CONSULTORIO")}
        ${hrCheck("DOMICILIARIA", data.attentionType === "DOMICILIARIA")}
        ${hrCheck("EMERGENCIAS", data.attentionType === "EMERGENCIAS")}
        ${hrCheck("REFERENCIA", data.attentionType === "REFERENCIA")}
      </div>
      <table class="hr-fields">
        <tr>
          <td class="hr-lbl">Ap. paterno:</td><td class="hr-val">${safe(patient.paternal)}</td>
          <td class="hr-lbl">Ap. materno:</td><td class="hr-val">${safe(patient.maternal)}</td>
        </tr>
        <tr><td class="hr-lbl">Nombres:</td><td class="hr-val" colspan="3">${safe(patient.names)}</td></tr>
        <tr><td class="hr-lbl">Domicilio:</td><td class="hr-val" colspan="3">${safe(data.address)}</td></tr>
      </table>
      ${hrDiagnosisBlock(data, 3)}
      <div class="hr-section-title">OTRAS PRESTACIONES</div>
      <div class="hr-services">
        ${ambulatorioServices.map((s, i) => `<div>${safe(s)} <span class="hr-box">${data["service" + i] === "on" ? "X" : ""}</span></div>`).join("")}
      </div>
      <table class="hr-med-table">
        <thead>
          <tr>
            <th rowspan="2">MEDICAMENTOS E INSUMOS<br><small>(Nombre generico, FF y Concentracion)</small></th>
            <th rowspan="2">INDICACIONES<br><small>(Cant., Frec., Tiempo y Via)</small></th>
            <th colspan="2">CANTIDAD</th>
            <th colspan="2">VALOR</th>
          </tr>
          <tr><th>Rec.</th><th>Disp.</th><th>Unit.</th><th>Total</th></tr>
        </thead>
        <tbody>
          ${rows.map(m => `<tr>
            <td>${m ? safe(medicineLabel(m)) : ""}</td>
            <td>${m ? safe(m.instruction || "") : ""}</td>
            <td class="hr-center">${m ? safe(m.quantity) : ""}</td>
            <td></td><td></td><td></td>
          </tr>`).join("")}
        </tbody>
      </table>
      <table class="hr-cost-table">
        <tr>
          <td rowspan="2" class="hr-obs-cell"><strong>OBSERVACIONES</strong><div>${safe(data.observations)}</div></td>
          <td><strong>COSTO TOTAL:</strong></td>
        </tr>
        <tr><td><strong>COSTO TOTAL AL USUARIO:</strong></td></tr>
      </table>
      <div class="hr-signatures">
        <div class="hr-sign-area">
        ...
        <div class="hr-stamp">SELLO<br>ESTAB.</div>
        <div class="hr-sign-area">
          <div class="hr-sign-line"></div>
          <strong>Nombre y Firma del(la) paciente/acompanante</strong>
          <div class="hr-ci-row"><em>C.I.:</em><div class="hr-ci-line"></div></div>
        </div>
      </div>
      <p class="hr-legal-note">El prescriptor y dispensador certifican la veracidad de la informacion declarada en este documento medico legal. El usuario certifica haber recibido los medicamentos senalados.</p>
    </div>
  `;
}

function renderHalfInternadoNative(data) {
  const requestDate = splitDate(data.requestDate);
  const birthDate = splitDate(data.birthDate);
  ...
  return `
    <div class="hr-recipe hr-int">
      <div class="hr-header">
        ...
        <div class="hr-title">RECETARIO / RECIBO<br>ATENCION DEL PACIENTE INTERNADO</div>
        <div class="hr-panel">
          <div><strong>Nº DE EXPEDIENTE CLINICO:</strong> <span>${safe(data.clinicalRecord)}</span></div>
          <div><strong>SISTEMA UNICO DE SALUD:</strong> <span>${data.patientType === "SUS" ? "S.U.S" : ""}</span></div>
          <div><strong>VENTA:</strong> <span>${safe(patientSale)}</span></div>
      ...
      <table class="hr-fields">
        ...
        <tr>
          <td class="hr-lbl">Nombres:</td><td class="hr-val">${safe(patient.names)}</td>
          <td class="hr-lbl">SEXO:</td>
          <td><span class="hr-box">${data.sex === "F" ? "X" : ""}</span> F &nbsp; <span class="hr-box">${data.sex === "M" ? "X" : ""}</span> M</td>
        </tr>
        <tr>
          <td class="hr-lbl">Domicilio:</td><td class="hr-val" colspan="2">${safe(data.address)}</td>
          <td>
            <div class="hr-date-inline"><strong>F.NAC.</strong> ${safe(birthDate.day)}/${safe(birthDate.month)}/${safe(birthDate.year)}</div>
            <div class="hr-date-inline"><strong>F.SOL.</strong> ${safe(requestDate.day)}/${safe(requestDate.month)}/${safe(requestDate.year)}</div>
          </td>
        </tr>
      </table>
      <div class="hr-attention-row">
        <strong>INGRESO:</strong>
        ${hrCheck("Por referencia", data.admissionType === "Por referencia")}
        ${hrCheck("Por serv. emergencia", data.admissionType === "Por servicio de emergencia")}
        ${hrCheck("Por trab. de parto", data.admissionType === "Por trabajo de parto")}
        ${hrCheck("Por cons. externo", data.admissionType === "Por consultorio externo")}
      </div>
      <table class="hr-fields">
        <tr>
          <td class="hr-lbl">Serv. de ingreso:</td>
          <td class="hr-val">${safe(data.admissionService || data.service)}</td>
          <td class="hr-lbl">F. ingreso:</td>
          <td><span class="hr-date-inline">${safe(admissionDate.day)}/${safe(admissionDate.month)}/${safe(admissionDate.year)}</span></td>
        </tr>
        <tr>
          <td class="hr-lbl">Serv. de alta:</td>
          <td class="hr-val">${safe(data.dischargeService)}</td>
          <td class="hr-lbl">F. egreso:</td>
          <td><span class="hr-date-inline">${safe(dischargeDate.day)}/${safe(dischargeDate.month)}/${safe(dischargeDate.year)}</span></td>
        </tr>
      </table>
      ${hrDiagnosisBlock(data, 2)}
      <div class="hr-section-title">Procedimientos</div>
      <div class="hr-services">
        ${internadoProcedures.map((p, i) => `<div>${safe(p)} <span class="hr-box">${data["procedure" + i] === "on" ? "X" : ""}</span></div>`).join("")}
      </div>
      <table class="hr-med-table hr-med-noindicaciones">
        <thead>
          <tr>
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("./sw.js?v=manual-20260526-84").catch((error) => {
